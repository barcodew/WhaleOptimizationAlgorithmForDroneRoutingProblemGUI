...
public class DRP_FullC {
    ...
    static class ScenarioResult {
        ...
        void calculate() {
            ...
            bestDistance = (bestFitness > 0) ? (1.0 / bestFitness) - 1 : 0;
    ...
    public static void main(String[] args) {
        ...
        int totalRuns = 1; // jumlah run per skenario
    ...
    static void buildFullRoute(int[] permutation, double[][] adj,
        ...
        result.totalEnergy = totalDist * ALPHA;
    }
    ...
    static void exportKonvergensiCSV(ArrayList<ScenarioResult> results, String filename) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(filename))) {
            ...
            int no = 1;
            for (ScenarioResult s : results) {
                pw.printf(",P%d_G%d_%s_best", s.popSize, s.maxIter, s.datasetName);
                no++;
            }
            pw.println();
            ...
            // Data: ambil run terbaik dari tiap skenario
            for (int t = 0; t < maxLen; t++) {
                pw.print(t + 1);
                for (ScenarioResult s : results) {
                    // Cari run terbaik
                    RunResult bestRun = s.runs[0];
                    for (RunResult r : s.runs) {
                        if (r.fitness > bestRun.fitness)
                            bestRun = r;
                    }

                    if (t < bestRun.convergence.length) {
                        pw.printf(",%.6f", bestRun.convergence[t]);
                    } else {
                        pw.print(",");
                    }
                }
            ...
            System.out.println("✓ Konvergensi disimpan: " + filename);
        } catch (Exception e) {
