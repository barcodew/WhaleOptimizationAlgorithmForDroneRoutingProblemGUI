...
public class DroneRoutingGUIgacor extends Application {
    ...
    double droneScreenX, droneScreenY, targetScreenX, targetScreenY;
    ...
    ScrollPane buildRightPanel(Stage stage) {
        ...
        Button b1 = new Button("Muat Dataset (.csv)");
        styleBtn(b1, "#1a5276");
        b1.setOnAction(e -> {
            FileChooser fc = new FileChooser();
            fc.setTitle("Pilih Dataset");
            fc.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV", "*.csv"));
            File f = fc.showOpenDialog(stage);
            if (f != null && loadDataset(f.getAbsolutePath())) {
        ...
        p.getChildren().addAll(b1, b2, sep());
        ...
        p.getChildren().addAll(lblFitness, lblDistance, lblEnergy, lblRecharge, sep());
        ...
        animTimer = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                double cx = droneScreenX + (targetScreenX - droneScreenX) * animProgress,
                        cy = droneScreenY + (targetScreenY - droneScreenY) * animProgress;
                double[] f = gn(bestRoute[currentSegment]), t = gn(bestRoute[currentSegment + 1]);
                if (f != null && t != null) {
                    double sd = Math.sqrt(Math.pow(f[1] - t[1], 2) + Math.pow(f[2] - t[2], 2));
                    double bat = segmentStartBattery - sd * ALPHA * animProgress;
                    updateBat(Math.max(0, bat));
                }
    ...
    void setupSeg() {
        ...
        if (f == null || t == null)
            return;
        droneScreenX = toScreenX(f[1]);
        droneScreenY = toScreenY(f[2]);
        targetScreenX = toScreenX(t[1]);
        targetScreenY = toScreenY(t[2]);
